...
from province import province_coord  # {"North": [...], ...}

# ฟังก์ชันสร้าง GeoJSON สำหรับภูมิภาค
def generate_geojson_for_region(avg_data, monthly_data, region_multipolygon, region_name, year):
    return {
        "type": "Feature",
        "geometry": json.loads(gpd.GeoSeries([region_multipolygon]).to_json())["features"][0]["geometry"],
        "properties": {
            "year": year,
...
def process_year(year):
    ...
    # === ประเทศ ===
    avg_data_country, country_geom = calculate_weighted_average_country(shapefile, data)
    monthly_data_country = calculate_monthly_averages(data, country_geom)
    country_feature = generate_geojson_for_country(avg_data_country, monthly_data_country, country_geom, year)

    # === ภูมิภาค ===
    region_dict = province_coord()  # {'North': [...], ...}
    region_features = []

    for region_name, province_list in region_dict.items():
        region_provinces = shapefile[shapefile['NAME_1'].isin(province_list)]
        ...
        region_multipolygon = MultiPolygon(polygons)

        avg_data_region, _ = calculate_weighted_average_region(shapefile, data, region_multipolygon)
        monthly_data_region = calculate_monthly_averages_region(data, region_multipolygon)

        region_feature = generate_geojson_for_region(
            avg_data_region, monthly_data_region, region_multipolygon,
            region_name, year
        )
...
#     # ลูปผ่านแต่ละภูมิภาค
#     for i, region_geom in enumerate(region_coords):  # region_geom ตอนนี้เป็น MultiPolygon แล้วจาก province.py
#         avg_data_region, _ = calculate_weighted_average_region(shapefile, data, region_geom)
#         monthly_data_region = calculate_monthly_averages_region(data, region_geom)
