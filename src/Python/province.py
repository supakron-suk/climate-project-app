...
def province_coord():
    ...
    province_Middle = [
        "Bangkok", "Kamphaeng Phet", "Chai Nat", "Nakhon Nayok", "Nakhon Pathom", "Nakhon Sawan",
    ...
    ne_list = [(name, gdf[gdf['NAME_1'] == name].geometry.unary_union, "ภาคตะวันออกเฉียงเหนือ") for name in province_NE]
    north_list = [(name, gdf[gdf['NAME_1'] == name].geometry.unary_union, "ภาคเหนือ") for name in province_North]
    south_list = [(name, gdf[gdf['NAME_1'] == name].geometry.unary_union, "ภาคใต้") for name in province_South]
    middle_list = [(name, gdf[gdf['NAME_1'] == name].geometry.unary_union, "ภาคกลาง") for name in province_Middle]
    east_list = [(name, gdf[gdf['NAME_1'] == name].geometry.unary_union, "ภาคตะวันออก") for name in province_East]
    west_list = [(name, gdf[gdf['NAME_1'] == name].geometry.unary_union, "ภาคตะวันตก") for name in province_West]
