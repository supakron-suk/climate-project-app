...
def spatial_overlays(df1, df2, how='intersection', reproject=True):
    ...
    if how=='intersection':
        ...
        df1['bbox'] = df1.geometry.apply(lambda x: x.bounds)
        df1['sidx']=df1.bbox.apply(lambda x:list(spatial_index.intersection(x)))
        df1['new_g'] = df1.apply(lambda x: functools.reduce(lambda x, y: x.difference(y).buffer(0),
        ...
        dfsym['new_g'] = dfsym.apply(lambda x: functools.reduce(lambda x, y: x.difference(y).buffer(0),
