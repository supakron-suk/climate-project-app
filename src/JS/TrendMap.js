...
const groupedTrends = trends.reduce((acc, curr) => {
  const key = curr.name; // ใช้ชื่อพื้นที่เป็นคีย์
  if (!acc[key]) acc[key] = [];
  acc[key].push({ year: curr.year, temperature: curr.temperature }); // เก็บปีและอุณหภูมิในแต่ละพื้นที่
  return acc;
}, {});

//console.log("Grouped Trends:", groupedTrends); 

  // คำนวณค่า slope ของแต่ละพื้นที่ และสร้างข้อมูล GeoJSON
  const features = Object.entries(groupedTrends).map(([area, data]) => {
    const slope = calculateModifiedTheilSenSlope(data); // คำนวณ slope
    const roundedSlope = parseFloat(slope.toFixed(2)); // ปัดค่า slope ให้มีทศนิยม 2 ตำแหน่ง
    return {
      type: "Feature", // ระบุประเภทข้อมูล GeoJSON
      geometry: geometryMap[area], // นำข้อมูลตำแหน่งทางภูมิศาสตร์มาใส่
...
//       if (x2 === x1) {
//         continue;  // ข้ามการคำนวณนี้ เพราะจะทำให้เกิด Infinity หรือ -Infinity
//       }

//       const slope = (y2 - y1) / (x2 - x1);

//       // ตรวจสอบผลลัพธ์ที่เป็น Infinity หรือ -Infinity
//       if (slope === Infinity || slope === -Infinity) {
//         continue;  // ข้ามกรณีที่เป็น Infinity
//       }
...
//   // ถ้าไม่พบการคำนวณใดๆ ก็คืนค่า 0 หรือค่าอื่นที่เหมาะสม
//   if (slopes.length === 0) {
//     return 0;
//   }
